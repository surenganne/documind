...
import type { EvalConfig, ModelProviderConfig } from '../types';
...
const PROVIDER_OPTIONS = {
  llm: [
    { name: 'bedrock', label: 'Amazon Bedrock', models: ['us.anthropic.claude-sonnet-4-5-20250929-v1:0', 'us.anthropic.claude-3-5-sonnet-20241022-v2:0'] },
...
function ModelProvidersSection() {
  ...
  // Add form state
  const [formType, setFormType] = useState<'llm' | 'embedding'>('embedding');
  const [formProvider, setFormProvider] = useState('bedrock');
  const [formModel, setFormModel] = useState('amazon.titan-embed-text-v2:0');
  ...
  const handleProviderChange = (provName: string) => {
    ...
    const opts = PROVIDER_OPTIONS[formType];
    const prov = opts.find(p => p.name === provName);
      ...
      setTestResults(prev => ({
        ...
        [provider.id]: {
          ...
          message: data.success
            ? (data.dimensions ? `OK — ${data.dimensions} dims` : data.response_preview || 'OK')
            : (data.error || 'Failed'),
        },
      }));
    } catch {
        ...
        <form onSubmit={handleAdd} className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                value={formType}
                onChange={(e) => handleTypeChange(e.target.value as 'llm' | 'embedding')}
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="formIsDefault" className="text-sm text-slate-700 cursor-pointer">Set as default for this type</label>
          </div>
          <button
            type="submit"
            disabled={saving}
